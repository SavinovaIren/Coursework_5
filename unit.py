...
class BaseUnit(ABC):
    ...
    def _count_damage(self, target: BaseUnit) -> int:
        ...
        if target.stamina > target.armor.stamina_per_turn:
            target_amor = target.armor.defence * target.unit_class.armor
            target.stamina -= target.armor.stamina_per_turn
        else:
            target_amor = 0
        damage = target.get_damage(unit_damage - target_amor)
        return damage
    ...
    def add_stamina(self, stamina_point):
        if self.stamina + stamina_point * self.unit_class.stamina > self.unit_class.max_stamina:
            self.stamina = self.unit_class.max_stamina
        else:
            self.stamina += stamina_point * self.unit_class.stamina
